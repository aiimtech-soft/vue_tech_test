...
export class BrokerApplicationsListController {
  ...
  async find(
    ...
    const loanAvgAmt: Application[] = await this.applicationEntity.findAll({
      attributes: [
        [Sequelize.literal(`SUM(loan_amount)`), 'loanAmount'],
      ],
      where: { status: 'funded' }
    });
    ...
    const applications = await this.applicationEntity.findAll({
      attributes: [
        ...
        'loanAmount',
        'outgoingAddress',
        [Sequelize.literal(`CASE WHEN loan_amount > ${loanAvgAmt[0].loanAmount} THEN 'Above' WHEN loan_amount = ${loanAvgAmt[0].loanAmount} THEN 'actual' ELSE 'Below' END`), 'loanAvg'],
      ],
      where: whereOptions,
      include: relatedData,
    });
